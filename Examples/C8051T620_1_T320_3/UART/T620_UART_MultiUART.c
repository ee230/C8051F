...
// running at 48 MHz.
//
// The 'T620/1 does not have a clock multiplier, but this function is
...
void Port_Init (void)
{
   P0MDOUT |= 0x11;                    // Set TX (UART1) and TX (UART0) to
                                       // push-pull output
   P2MDOUT |= 0x04;                    // Set LED to push-pull output
