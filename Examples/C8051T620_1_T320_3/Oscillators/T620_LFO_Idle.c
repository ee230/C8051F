...
// savings mode operation on the T620/1. The MCU will sleep in low power idle
// mode with only the low frequency oscillator and timer T0 running. When
// the MCU wakes up it will switch to a 12 MHz clock and turn on an LED
// and wait 100 ms. The 100 ms delay also uses T0 and the low frequency
// oscillator but does not use idle mode or interrupts.
//
// How To Test:
//
// 1) Download code to a 'T620 on a 'T62x Development Board.
// 2) Make sure the P2.3 LED jumper is in place on J10.
// 3) Run the code.
// 4) The P2.3 LED should blink once a second.
//
//
