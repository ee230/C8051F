...
// How To Test:
//
// 1) Download code to a 'T620 device on a C8061T62x development board.
// 2) Populate the RX_COMM and TX_COMM jumpers on J11.
// 3) Populate J14 to connect the capacitors to VREF.
// 4) Connect a USB cable from the COMM port to a PC (CP210x USB-to-UART drivers
...
// For the 'T620, the AMX0P settings correspond to the following port pins:
//
//    AMX0P      Port Pin
//    0x08         P2.0
//    0x09         P2.1
...
void main (void)
{
   ...
   while (1)
   {
      ...
      Timer0_wait(10);                 // Wait before displaying new values
   }
}
...
void Port_Init (void)
{
   P0SKIP = 0x80;                      // Skip VREF
   P2SKIP = 0x3F;                      // Skip all analog pins

   P0MDOUT |= 0x10;                    // Enable TX0 as a push-pull output

   P0MDIN &= ~0x80;                    // P0.7 in analog mode
   P2MDIN &= ~0x3F;                    // Set ADC0 inputs as analog
