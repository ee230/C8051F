//-----------------------------------------------------------------------------
// T620_SPI0_EEPROM_Polled_Mode.c
//-----------------------------------------------------------------------------
...
// Program Description:
//
// This program accesses a SPI EEPROM using polled mode access. The 'T620 MCU
...
// connections of the 'T620 MCU are shown here:
//
...
//
// Method1:
// 1) Download the code to a 'T620 device in a 'T62x mother board that is
//    connected as above.
...
//
// Method2 (optional):
// 1) Download code to a 'T620 device in a 'T62x mother board that is connected
//    as above.
// 2) Connect a straight USB cable from the COMM USB port on the mother board
//    to a PC (CP210x drivers must be already installed).
// 3) Install the RX_COMM and TX_COMM jumpers on J11.
...
void Oscillator_Init (void)
{
   int i = 0;
   OSCICN = 0x83;
   CLKMUL = 0x80;                      // This is unnecessary on the 'T620/1 as
...
//
// P2.2  -  Skipped,     Push-Pull,  Digital (LED D1 on Target Board)
//
//
//-----------------------------------------------------------------------------
void Port_Init (void)
