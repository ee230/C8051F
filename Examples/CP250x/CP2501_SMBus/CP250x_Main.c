...
//
// The purpose of this firmware is to show how to call the SMBus functions 
...
// SMBus slave address of the touch-screen module
#define  SLAVE_ADDR              0xB0

// Commands to send touch-screen module over SMBus
...
U16 SMBus_Buffer[4];         // X1, Y1, X2, Y2 coordinates from touch screen
...
void main (void)
{
   ...
   SMBus_clock.U32 = SMBUS_CLOCK;
   CP250x_SMBus_Master_Init (SMBus_clock, 0xA0);
   if (CP250x_Status != FUNC_SUCCESS)
   {
   ...
   while (1)
   {
      ...
      {
         // Send a command to SMBus module to retrieve touch data
         CP250x_SMBus_Master_Transfer (SMBUS_FUNC_READ, BUFFER_SIZE, SLAVE_ADDR);
         read_started = 1;
      ...
      if (read_started)
      {
         ...
         // If the SMBus read has completed successfully
         if (CP250x_Status == FUNC_SMBUS_IDLE) 
         {
            read_started = 0;
            buf_ptr = (U8 *) SMBus_Buffer;
