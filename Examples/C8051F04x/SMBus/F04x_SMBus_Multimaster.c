...
// Program Description:
//
// Example software to demonstrate the C8051F04x SMBus interface in
...
//      - the 'F04x SMBus master should detect any conflict using ARBLOST
//      - the 'F04x SMBus master should give up the bus, if necessary, and
//        switch to slave receiver mode
//      - the 'F04x SMBus master should transmit after the other master is done
// - Pinout:
//    P0.0 -> SDA (SMBus)
//    P0.1 -> SCL (SMBus)
//
...
// How To Test:
//
// 1) Download code to a 'F04x device that is connected to another C8051Fxxx
//    SMBus multimaster.
...
//
//      'F04x Multimaster                     Other C8051Fxxx Multimaster
//         LED (blinks) <------------------------------ Switch
//              SW --------------------------------> LED (blinks)
//
...
void MAIN (void)
{
   ...
   Timer4_Init ();                     // Configure and enable Timer3
...
void Timer4_ISR (void) interrupt 16
{
   ...
   TF4 = 0;                            // Clear Timer3 interrupt-pending flag
   SMB_BUSY = 0;                       // Free bus
}
