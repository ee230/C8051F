...
// Program Description:
//
// This program presents an example of use of the Timer0 of the C8051F2xx's
// in 8-bit reload timer mode. It uses the 'F2xxDK as HW platform and blinks
...
//  system clock (SYSCLK, in this case the internal osc. @16MHz/8) with a
// 1:12 prescaler.
// The interrupt routine then toggles the LED. To execute the test please
// follow these steps:
//
// 1) Open the F2xx_Timer0_8bitReloadTimer.c file in the Silicon Labs IDE.
// 2) If a different LED blink rate is required change the following constant:
//       -> LED_TOGGLE_RATE in milliseconds.
// 3) Compile and download the code to a 'F2xx
