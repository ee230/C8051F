...
// Program Description:
//
// This program presents an example of use of the Timer0 of the C8051F2xx's in
...
//
// This code uses the 'F2xxDK as HW platform.
//
// Pinout:
//
//    P0.2 -> /INT0
//
//    P0.4 -> T0 (Timer0 External Input)
//
//    P1.5 -> GTE (digital, push-pull)
//
//    P2.1 -> SIGNAL (digital, push-pull)
//
//    P2.4 -> LED
//    P2.5 -> BUTTON1 (switch)
//
...
// How To Test:
//
// 1) Open the F2xx_Timer0_13bitExtTimer.c file in the Silicon Labs IDE.
// 2) To change the number of input pulses/interrupt, modify
//    PULSES_PER_TOGGLE.
// 3) To change the speed of the SIGNAL waveform, modify
//    SOFTWARE_DELAY
// 4) Compile the project
// 5) Download code to a 'F2xx device
// 6) Verify the P2.5 and LED jumpers are populated on the 'F2xx TB.
// 7) Connnect the following pins:
//
// P0.4 <--> P2.1 (T0 with SIGNAL) (pin 13 to pin 26 on the 'F2xx TB)
// P0.2 <--> P1.5 (/INT0 with GTE) (pin 15 to pin 6 on the 'F2xx TB)
//
// 8) Run the code
...
sbit SIGNAL = P2^1;                    // SIGNAL is used to input pulses into
                                       // T0 pin
sbit BUTTON = P2^5;                    // Button that enables counting
...
void Port_Init (void)
{
   PRT2CF = 0x12;                      // Set LED and SIGNAL as output
   PRT1CF = 0x20;                      // Set GTE as output
   PRT0MX = 0x14;                      // T0 and INT0 available at the pins
}
