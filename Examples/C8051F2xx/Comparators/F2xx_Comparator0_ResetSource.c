...
// spins in an infinite while() loop.  If the button on P2.5 (on the target
// board) is pressed, then the comparators negative input (CP0-) will be  
// pulled above the comparator's positive input (CP0+).  When this happens,  
...
// How To Test:
//
// 1) Compile and download code to a 'F206 target board
// 2) On the target board, connect P1.1/CP0- to P2.5 and P1.0/CP0+ to P2.4
//    This pins are available on the J2 header
//    P1.1 - pin 10
//    P2.5 - pin 22
//    P1.0 - pin 9
//    P2.4 - pin 21
// 3) Run the code and verify that the LED is on
// 4) Press the P2.5 switch (this causes a comparator reset)
...
#define SYSCLK             16000000    // Clock speed in Hz
#define COMPARATOR_SETTLE  10          // Settle time in us

sbit LED    = P2^4;                    // Green Led
sbit SWITCH = P2^5;                    // Push-button switch on board
...
// P2.5   digital   open-drain    Switch
//-----------------------------------------------------------------------------

void PORT_Init (void)
{
   PRT2CF |= 0x10;                     // LED configured for push-pull
   P1MODE &= ~0x03;                    // Comparator inputs configured for
                                       // analog input
}
...
{
   CPT0CN = 0x80;                      // Comparator0 enabled
