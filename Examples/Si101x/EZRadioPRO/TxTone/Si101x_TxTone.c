...
// set the BW to 10 MHz per division.
//
// Target:
//    si101x
//
...
//-----------------------------------------------------------------------------
//  T0 timeout defined as not T0 enable
//-----------------------------------------------------------------------------
#define TIMEOUT_T0   (!ET0)
//-----------------------------------------------------------------------------
// RF Switch enabled by default
//   Needed for +20 daughtercards only.
//   But does not cause any problem with +13 dB boards.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
SBIT(NSS1, SFR_P1, 3);                 // Si101x
//-----------------------------------------------------------------------------
// SI1000 Daughter Card Connections
//-----------------------------------------------------------------------------
SBIT(IRQ, SFR_P0,1);
SBIT(SDN, SFR_P1,6);                   // Si101x
//-----------------------------------------------------------------------------
...
void main (void)
{
   ...
   // Actual power depends on Si100x variant power rating.
   ...
   // RF Switch enabled by default
   // Needed for +20 daughter cards only.
...
void PortInit (void)
{
   ...
   P1MDOUT |= 0x0D;                    // MOSI, SCK, NSS, push-pull
   SFRPAGE  = CONFIG_PAGE;
   P1DRV   |= 0x0D;                    // MOSI, SCK, NSS, high current mode
   SFRPAGE  = LEGACY_PAGE;

   // Init SDN
   P1MDOUT |= 0x40;                    // SDN push-pull
   SFRPAGE  = CONFIG_PAGE;
   P1DRV   |= 0x40;                    // SDN High-Current Mode
   SFRPAGE  = LEGACY_PAGE;
...
void delay (U16 ticks)
{
   ...
   TMOD      = 0x01;    // T0 uses pre-scaler
   CKCON     = 0x02;    // divide by 48 pre-scaler
