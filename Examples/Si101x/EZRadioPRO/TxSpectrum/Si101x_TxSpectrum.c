...
//-----------------------------------------------------------------------------
SBIT(IRQ, SFR_P0,1);
SBIT(SDN, SFR_P1,6);                   // all variants
//-----------------------------------------------------------------------------
...
void setTxPower (U8 power)
{
   ...
   version = phyRead(EZRADIOPRO_DEVICE_VERSION);

   if(version>3)
   {
      phyWrite(EZRADIOPRO_TX_POWER, 0x18|power); // a0/B0/B1
   }
   else
   {
      phyWrite(EZRADIOPRO_TX_POWER, (power>>1)); // V2
   }
}
//-----------------------------------------------------------------------------
