...
// How To Test:
//
// 1) Open the F30x_Timer2_two_8bitTimers.c file in the Silicon Labs IDE
// 2) If necessary change the number of interrupts to toggle an output:
//    LED_TOGGLE_RATE
//    SIGNAL_TOGGLE_RATE
// 3) Compile and download the code
// 4) Verify the LED pins of J3 are populated on the 'F30x TB.
// 5) Run the code
// 6) Verify that the LED (P0.2) is blinking and SIGNAL (P0.7) is toggling.
//
//
// FID:            30X000033
// Target:         C8051F30x
...
sbit SIGNAL = P0^7;                    // SIGNAL to be used also by Timer2
