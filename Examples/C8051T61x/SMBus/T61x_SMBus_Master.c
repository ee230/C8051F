...
// Program Description:
//
// Example software to demonstrate the C8051T61x SMBus interface in
// Master mode.
...
// How To Test:
//
// 1) Download code to a 'T61x device that is connected to a SMBus slave.
// 2) Run the code:
//         a) The test will indicate proper communication with the slave by
//            toggling the LED on and off each time a value is sent and
//            received. Note that the LED toggled at 104 Hz in this example
//            and will be difficult to see with the naked eye.
