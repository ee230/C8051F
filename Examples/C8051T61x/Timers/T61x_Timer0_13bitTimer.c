...
// Program Description:
//
// This program presents an example of use of the Timer0 of the C8051T61x's
// in 13-bit counter/timer mode. It uses the 'T610DK as HW platform and blinks
// an LED. To do so it used an interrupt driven by the Timer0 which toggles the
// LED every 100msec.
//
// Pinout:
//
...
// follow these steps:
//
// 1) Open the T61x_Timer0_13bitTimer.c file in the Silicon Labs IDE.
// 2) If a different LED blink rate is required change the following constant:
//       -> LED_TOGGLE_RATE in milliseconds
// 3) Compile and download the code to a 'T61x device.
// 4) Verify the LED pins of J9 are populated on the 'T61x TB.
// 5) Run
