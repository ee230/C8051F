...
// How To Test:
//
// 1) Download code to a 'T61x device that is connected to a UART transceiver
// 2) Verify the TX and RX jumpers are populated on J10.
// 3) Verify the J9 (LED - P1.0) jumper is not populated.
...
// Port1 ADC Inputs
U8 PIN_TABLE[ANALOG_INPUTS] = {0,1,2,3,4,5};
...
void main (void)
{
   ...
   while (1)
   {
      ...
      for(i = 0; i < ANALOG_INPUTS; i++)
      {
         ...
         measurement =  (RESULT[i] * 1800) / 1023; 
...
void ADC0_Init (void)
{
   ...
   REF0CN = 0x10;                      // Use precision internal VREF = 1.8V
