...
// 2) On the target board, connect P0.0 to P1.0 and P0.1 to P0.7
// 3) Run the code and verify that the LED is on
// 4) Press the P1.0 switch (this causes a comparator reset)
...
sbit LED    = P0^7;                    // Green Led
sbit SWITCH = P1^0;                    // Push-button switch on board
...
void PORT_Init (void)
{
   ...
   P0MDIN  &= ~0x03;                   // Comparator 0 inputs (P0.0 and P0.1)
                                       // are analog inputs.
   P0MDOUT  = 0x80;                    // LED is push-pull output
}
