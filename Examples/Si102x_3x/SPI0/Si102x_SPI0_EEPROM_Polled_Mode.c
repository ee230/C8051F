//-----------------------------------------------------------------------------
// Si102x_SPI0_EEPROM_Polled_Mode.c
//-----------------------------------------------------------------------------
...
// Program Description:
//
// This program accesses an SPI EEPROM using polled mode access. The MCU
...
//    board that is connected as shown above.
// 2) Run the code. LED1 will blink fast during the write/read/verify
//    operations.
// 3) If the verification passes, LED1 will blink slowly. If it fails,
//    LED1 will be OFF.
//
// Method2 (optional):
// 1) Download the code to an Si102x/3x device on a UPMP-F960-MLCD development
...
//    verification passes, LED1 will blink slowly. If it fails, LED1 will
//    be OFF.
//
// Target:         Si102x/3x
// Tool chain:     Generic
...
void PORT_Init (void)
{
   ...
   P0MDOUT |= 0x01;                     // Make the LED push-pull
