//-----------------------------------------------------------------------------
// T622_SPI0_EEPROM_Polled_Mode.c
//-----------------------------------------------------------------------------
...
// Program Description:
//
// This program accesses a SPI EEPROM using polled mode access. The 'T622 MCU
...
// connections of the 'T622 MCU are shown here:
//
...
//
// P1.2 - LED        (digital output, push-pull)
//
//
// How To Test:
//
// Method1:
// 1) Download the code to a 'T622 device in a 'T62x mother board that is
//    connected as above.
// 2) Remove the switch jumper on P0.1 (J9).
...
//
// Method2 (optional):
// 1) Download code to a 'T622 device in a 'T62x mother board that is connected
//    as above.
// 2) Remove the switch jumper on P0.1 (J9).
// 3) Connect a straight USB cable from the COMM USB port on the mother board
//    to a PC (CP210x drivers must be already installed).
// 4) Install the RX_COMM and TX_COMM jumpers on J11.
// 5) On the PC, open HyperTerminal (or any other terminal program) and connect
//    to the COM port at <BAUDRATE> and 8-N-1
// 6) HyperTerminal will print the progress of the write/read operation, and in
...
// Parameters   : None
//
// This function initializes the system clock at 24 MHz.
//
//-----------------------------------------------------------------------------
void Oscillator_Init (void)
{
   int i = 0;
   OSCICN = 0x83;
   CLKMUL = 0x80;                      // This is unnecessary on the 'T622/1 as
                                       // there is no longer a Clock
                                       // Multiplier, but this code is still
                                       // necessary for the 'F34x.
...
//
// P1.2  -  Skipped,     Push-Pull,  Digital (LED D1 on Target Board)
//
//
//-----------------------------------------------------------------------------
void Port_Init (void)
