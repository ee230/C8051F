...
// Program Description:
//
// This program configures a C8051T622 as a SPI Slave. The 'T622 MCU
...
// 2) Verify that the P1.2 LED pin on jumper J10 is populated.
// 3) Remove the P0.1 switch jumper (J9).
// 4) Run the code.
...
// at 24 MHz (using 48 MHz Internal Oscillator / 2).
//
//-----------------------------------------------------------------------------
void Oscillator_Init (void)
{
   CLKSEL = 0x02;                      // Set the SYSCLK to 24 MHz
}
...
//
// P1.2  -  Skipped,     Push-Pull,  Digital (LED D4 on Target Board)
//
//-----------------------------------------------------------------------------
void PORT_Init (void)
{
   P0MDOUT = 0x02;                     // Make MISO push-pull
   P1MDOUT = 0x04;                     // Make the LED push-pull

   P1SKIP = 0x04;                      // Skip the LED (P1.2)
