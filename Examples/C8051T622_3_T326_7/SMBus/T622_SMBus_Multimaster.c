...
// Program Description:
//
// Example software to demonstrate the C8051T622/3 and 'T326/7 SMBus interface
...
//      - the 'T622/3 or 'T326/7 SMBus master should detect any conflict using
//        ARBLOST
//      - the 'T622/3 or 'T326/7 SMBus master should give up the bus, if
//        necessary, and switch to slave receiver mode
//      - the 'T622/3 or 'T326/7 SMBus master should transmit after the other
//        master is done
// - Pinout:
//    P0.0 -> SDA (SMBus)
//    P0.1 -> SCL (SMBus)
...
// How To Test:
//
// 1) Download code to a 'T622 device on a 'T62x mother board that is connected
//    to another C8051Fxxx/Txxx SMBus multimaster.
// 2) Remove the jumper on J9 for P0.1.
// 3) Verify the P1.0 SW2 jumper is populated on J9.
// 4) Run the code.  The test will indicate proper communication with the
//    other C8051Fxxx/Txxx device by blinking the LED on the other device on
//    and off each time SW is pressed, even if the buttons on both boards are
//    pressed at the same time.
//
//      'T622 Multimaster                    Other C8051Fxxx/Txxx Multimaster
//         LED (blinks) <------------------------------ Switch
//              SW --------------------------------> LED (blinks)
//
...
SBIT (SW, SFR_P1, 0);                  // Switch on P1.0

SBIT (LED, SFR_P1, 2);                 // LED on port P1.2
...
void main (void)
{
   ...
   OSCICN |= 0x03;                     // Set internal oscillator to 12 MHz
...
// be adjusted.
//
// Note: The P0.1 pin is shared with SW1, so the jumper on J9 must be removed.
//
//-----------------------------------------------------------------------------
void PORT_Init (void)
{
   ...
   P1MDOUT |= 0x04;                    // Make the LED (P1.2) a push-pull
                                       // output
