//-----------------------------------------------------------------------------
// T622_SMBus_Slave_Hardware_ACK.c
//-----------------------------------------------------------------------------
...
// in Slave mode with Hardware Acknowledge enabled.
...
//
// Release 1.1
//    -Corrected SMBus_Init function to set correct address mask (ES)
//    -19 JUN 2009
//
// Release 1.0
//    -Initial Revision (TP)
...
#include "compiler_defs.h"             // Compiler-specific macro definitions
...
#define  SMB_FREQUENCY  100000         // Target SMBus frequency
...
// - SMBus SCL timeout detection enabled
// - Hardware acknowledge enabled
//
//-----------------------------------------------------------------------------
void SMBus_Init (void)
{
   SMB0CF = 0x09;                      // Use Timer1 overflows as SMBus clock
                                       // source;
                                       // Enable slave mode;
...
// Parameters   : None
//
// Timer3 configured for use by the SMBus SCL low timeout detect feature as
// follows:
   ...
   if (ARBLOST == 0)
   {
      switch (SMB0CN & 0xF0)           // Decode the SMBus status vector
      {
         ...
         case  SMB_SRADD:
            ...
            {
               ACK = 1;                // ACK the received data from the
                                       // master WRITE
            }
            break;
...
// and re-enabled here to reset communication.
//
//-----------------------------------------------------------------------------
