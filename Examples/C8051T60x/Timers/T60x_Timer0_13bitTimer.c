...
// Program Description:
//
// This program presents an example of use of the Timer0 of the C8051T60x's
// in 13-bit counter/timer mode. It blinks an LED.
// To do so it uses an interrupt driven by the Timer0 which toggles the
// LED.
//
// Pinout:
//
...
// 3) Compile and download the code to a 'T60x device.
...
#define AUX2     0x1FFF - (TIMER_TICKS_PER_MS*LED_TOGGLE_RATE)
...
#if (CHIPNUM == 606)
 SBIT (LED1, SFR_P0, 2);               // LED1; 1 = on, 0 = off
#else
 SBIT (LED1, SFR_P0, 6);
#endif
