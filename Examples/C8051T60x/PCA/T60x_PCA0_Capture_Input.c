...
// How To Test:
//
// 1) Download code to a 'T60x device with P0.3 and P0.1 tied together.
// 2) Run the program for a few seconds, and then halt.
// 3) Verify that the variable "capture_period" is equal to 50 (=500us).
//
// Alternately:
// 1) Run a separate signal generator into the P0.3 pin.
// 2) Run the program for a few seconds, and then halt.
...
//    - Changed to use P0.3 as input so example runs on 'T606 also.
//

// Release 1.1 / 30 JUL 2008 (ADT)
...
void PORT_Init (void)
{
   XBR0    = 0x07;                     // Skip P0.0, P0.1, and P0.2
   XBR1    = 0x40;                     // Route CEX0 to P0.3,
