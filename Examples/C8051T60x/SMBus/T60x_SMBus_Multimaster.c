...
// Program Description:
//
// Example software to demonstrate the C8051T60x SMBus interface in
...
//      - the 'T60x SMBus master should detect any conflict using ARBLOST
//      - the 'T60x SMBus master should give up the bus, if necessary, and
//        switch to slave receiver mode
//      - the 'T60x SMBus master should transmit after the other master is done
// - Pinout:
//    P0.2 -> LED (T606)
//    P0.3 -> SW
//    P0.4 -> SDA (SMBus)
...
// 2) Download code to a 'T60x device that is connected to another C8051Fxxx
//    SMBus multimaster.
// 3) Run the code.  The test will indicate proper communication with the
...
//
//      'T60x Multimaster                     Other C8051Fxxx Multimaster
//         LED (blinks) <------------------------------ Switch
//              SW --------------------------------> LED (blinks)
//
...
//    - Changed code to use P0.4 and P0.5 for SMBus so it will run on 'T606
//
...
U32 NUM_ERRORS;                        // Counter for the number of errors.

// 16-bit SFR declarations

sbit SW = P0^3;                        // Switch on P0.7

#if (CHIPNUM == 606)
SBIT (LED, SFR_P0, 2);                 // LED on port P0.2
#else
SBIT (LED, SFR_P0, 6);
#endif

SBIT (SDA, SFR_P0, 4);                 // SMBus on P0.4
SBIT (SCL, SFR_P0, 5);                 // and P0.5
...
// P0.4   digital   open-drain    SMBus SDA
// P0.5   digital   open-drain    SMBus SCL
...
void PORT_Init (void)
{
   ...
   XBR0 = 0x0F;                        // Skip P0.0 through P0.3
